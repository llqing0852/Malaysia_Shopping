...
import com.tanie.fenxiao.entities.*;
import com.tanie.fenxiao.service.*;
...
import com.unstoppedable.common.*;
...
public class OrdersAction extends BaseAction {
    ...
    public void add() {
        ...
        Product findProduct = this.productService.findById(
                Product.class, pid);
        if (findProduct == null) {
            this.request.setAttribute("status", "0");
    ...
    public void save() {
        ...
        if (findProduct == null) {
            ...
            if ((loginUser == null) || (loginUser.getId() == null)) {
                ...
                try {
                    String payUrl = PayUtil.getPayUrl(userName, cardNo, newOrders.getNo(), newOrders.getMoney(), pid);

                    this.response.sendRedirect(payUrl);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
    ...
    public void indexList() {
        ...
        if (("1".equals(type)) || ("6".equals(type))) {
            ...
            hql = hql + " and status in(1,6)";
            countHql = countHql + " and status in(1,6)";
        }

        //hql = hql + " order by id desc";

        int count = this.ordersService.getTotalCount(countHql, new Object[0]);
    ...
    public void update() {
        ...
        try {
            if (this.orders == null) {
                ...
                Orders findorders = this.ordersService.findById(
                            ...
                            for (int j = 1; i > 0; j++) {
                                if (!StringUtils.isEmpty(leverNoArr[i])) {
                                    ...
                                    if (levelUser != null) {
                                        ...
                                        commission.setLevel(Integer.valueOf(j));
                                        commission.setRemark("第" + j + "级用户:编号【"
                                                + findOrders.getUser().getNo() + "】购买商品奖励");
                                        this.commissionService
                                                .saveOrUpdate(commission);
                                    }
                                }
