...
<head>
    ...
    <script type="text/javascript">
        ...
        function ordersSave(id, number) {
            window.location.href = "ordersSave?pid=" + id + "&num=" + number;
        }
    </script>
...
<section class="ww">
    ...
    <div class="sn-block mb30" id="sndistribution"
                ...
                <div class="wbox-flex" id="payInfoArea">
                    ...
                    <p><label class="w3">订单状态</label>：
                        <c:if test="${orders.status==0 }">未付款
                            <a href="javascript:ordersSave('${orders.productId }','${orders.productNum }')"
                               id="submitOrderDivId" class="sn-btn sn-btn-assertive sn-btn-big">付款</a>
                        </c:if>
                        <c:if test="${orders.status==1 }">已付款</c:if>
                        <c:if test="${orders.status==6 }">系统待确认</c:if>
                    </p>
